...
/* 
   ...
   - Sağ üstte geçen süre (saniye) estetik şekilde gösterilir.
*/
...
function initGame() {
  ...
  // Yeni: Oyun süresi için timerText ve startTime
  let timerText;
  let startTime;
  ...
  function preload() {
    ...
    // Düşman resmi: Eski kutu benzeri asset yerine farklı bir görsel
  ...
  function create() {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        enemy.setScale(1.5); // Buraya istediğiniz ölçeği ekleyin
    ...
    // Yeni: Timer Text - Sağ üst köşeye yerleştiriyoruz (config.width=800)
    startTime = this.time.now;
    timerText = this.add.text(config.width - 150, 10, 'Time: 0.00', { font: '20px Arial', fill: '#ffffff' });
  ...
  function update(time, delta) {
    ...
    // Güncelle: Timer text'i her karede güncelle
    let elapsedTime = (time - startTime) / 1000;
    timerText.setText('Time: ' + elapsedTime.toFixed(2));
    ...
    // Her 100 puan farkında beam hazır hale gelsin:
